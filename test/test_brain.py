# main.py
from test_functions import AudioProcessor, ChatBot
...
def main_loop():
    ...
    chat_bot = ChatBot(API_KEY, SYSTEM_MESSAGE, tools)

    list_pre_saved_voices = glob.glob(os.path.join(os.getcwd(),'pre_saved_voices','funny','*.wav'))
    index = random.randint(0,len(list_pre_saved_voices))
    pre_saved_audio_filenames_5 = [
        list_pre_saved_voices[index],
    ]

    pre_saved_audio_filenames_20 = [
        'internet_connection.wav',
        'let_me_think_for_a_moment.wav'
    ]
